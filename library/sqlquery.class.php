...
class SQLQuery {
    ...
    function delete($id){
        $query = 'delete from `'.$this->_table.'`where `id` = \''.mysqli_real_escape_string($this->_dbHandle,$id).'\'';
        return $this->query($query,1);
    }
    ...
    function query($query, $singleResult = 0) {
        ...
        if (preg_match("/select/i",$query)) {
            ...
            for ($i = 0; $i < $numOfFields; ++$i) {
                $indexed_db_rows = mysqli_fetch_field_direct($this->_result,$i);
                array_push($table,$indexed_db_rows->table);
                array_push($field,$indexed_db_rows->name);
            }
